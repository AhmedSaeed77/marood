                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="card-box table-responsive">
                            <h4 class="m-t-0 header-title"><b>{{ __('why_infraction.add_reason') }}</b></h4>
                            <p class="text-muted font-13 m-b-30">
                                <a class="btn btn-info" data-toggle="modal" data-target="#add-rerason">{{ __('why_infraction.add_reason') }}</a>
                            </p>
                            ...
                            <table id="datatable-colvid" class="table table-striped table-bordered">
                                ...
                                <tr>
                                    <th>#</th>
                                    <th>{{ __('why_infraction.arabic_reason') }}</th>
                                    <th>{{ __('why_infraction.english_reason') }}</th>
                                    <th>{{ __('why_infraction.buttons') }}</th>
                                </tr>
                                </thead>
                                <tbody>
                                @foreach($why as $i=>$w)
                                    <tr>
                                        ...
                                        <td>{{ $w->title_en }}</td>
                                        <td>
                                            <a data-toggle="modal" data-target="#del-page{{ $i }}" class="btn btn-danger">{{ __('why_infraction.delete') }}</a>
                                            <!-- Delete Modal -->
                                                ...
                                                <div class="modal-dialog modal-sm">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            ...
                                                            <h4 class="modal-title">{{ __('why_infraction.modal_title_delete', ['reason_name' => $w->title_ar]) }}</h4>
                                                        </div>
                                                        <div class="modal-body">
                                                            {{ __('why_infraction.confirm_delete', ['reason_name' => $w->title_ar]) }}
                                                        </div>
                                                        <div class="modal-footer">
                                                            <div class="row">
                                                                <div class="col-md-6 col-lg-6">
                                                                    <form action="{{ route('whyInfraction.destroy', $w->id) }}" method="post">
                                                                        @csrf
                                                                        <input type="hidden" name="_method" value="delete"/>
                                                                        <button type="submit" class="btn btn-danger">{{ __('why_infraction.delete') }}</button>
                                                                    </form>
                                                                </div>
                                                                <div>
                                                                    <button type="button" class="btn btn-default" data-dismiss="modal">{{ __('why_infraction.close') }}</button>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                            ...
                                            <a data-toggle="modal" data-target="#edit-reason{{ $i }}" class="btn btn-info">{{ __('why_infraction.edit') }}</a>
                                            <!-- Edit Modal -->
                                            <div class="modal fade" id="edit-reason{{ $i }}" role="dialog">
                                                <div class="modal-dialog ">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <button type="button" class="close" data-dismiss="modal">&times;</button>
                                                            <h4 class="modal-title">{{ __('why_infraction.modal_title_edit') }}</h4>
                                                        </div>
                                                        <div class="modal-body">
                                                            <form action="{{ route('whyInfraction.update', $w->id) }}" method="post">
                                                                @csrf
                                                                <input type="hidden" name="_method" value="put" />
                                                                <div class="row">
                                                                    <div class="form-group">
                                                                        <label class="col-md-3">{{ __('why_infraction.arabic_reason') }}</label>
                                                                        <div class="col-md-9">
                                                                            <input type="text" name="title_ar" value="{{ $w->title_ar }}" required class="form-control"/>
                                                                        </div>
                                                                    </div>
                                                                    <div class="form-group">
                                                                        <label class="col-md-3">{{ __('why_infraction.english_reason') }}</label>
                                                                        <div class="col-md-9">
                                                                    ...
                                                                    <div class="form-group">
                                                                        <div class="col-md-12">
                                                                            <button type="submit" class="btn btn-default" >{{ __('why_infraction.edit') }}</button>
                                                                            <button type="button" class="btn btn-danger" data-dismiss="modal" >{{ __('why_infraction.delete') }}</button>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </form>
    ...
    <div class="modal fade" id="add-rerason" role="dialog">
        <div class="modal-dialog ">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                    <h4 class="modal-title">{{ __('why_infraction.modal_title_add') }}</h4>
                </div>
                <div class="modal-body">
                    <form action="{{ route('whyInfraction.store') }}" method="post">
                        @csrf
                        <div class="row">
                            <div class="form-group">
                                <label class="col-md-3">{{ __('why_infraction.arabic_reason') }}</label>
                            ...
                            <div class="form-group">
                                <div class="col-md-12">
                                    <button type="submit" class="btn btn-default">{{ __('why_infraction.add') }}</button>
                                    <button type="button" class="btn btn-danger" data-dismiss="modal">{{ __('why_infraction.delete') }}</button>
                                </div>
                            </div>
                        </div>
