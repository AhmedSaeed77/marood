                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="card-box table-responsive">
                            <h4 class="m-t-0 header-title"><b></b>{{ $country->name_ar }}</h4>
                            <p class="text-muted font-13 m-b-30">
                                <a class="btn btn-default" data-toggle="modal" data-target="#add_area">{{ __('country.add_region') }}</a>
                            </p>
                                ...
                                <div class="modal-dialog ">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            ...
                                            <h4 class="modal-title">{{ __('country.modal_title_add') }}</h4>
                                        </div>
                                        <div class="modal-body">
                                            <form method="post" action="{{ route('area.store') }}">
                                                @csrf
                                                <input type="hidden" value="{{ $country->id }}" name="parent_id"/>
                                                <div class="form-group">
                                                    <label class="col-md-3">{{ __('country.name_arabic') }}</label>
                                                    <div class="col-md-9">
                                                        <input class="form-control" required name="name_ar" type="text" />
                                                    </div>
                                                </div>
                                                <div class="form-group">
                                                    <label class="col-md-3">{{ __('country.name_english') }}</label>
                                                    <div class="col-md-9">
                                                        <input class="form-control" name="name_en" type="text" />
                                                    </div>
                                                </div>
                                                <div class="form-group">
                                                    <label class="col-md-3">{{ __('country.latitude') }}</label>
                                                    <div class="col-md-9">
                                                        <input class="form-control" name="lat" type="number" />
                                                    </div>
                                                </div>
                                                <div class="form-group">
                                                    <label class="col-md-3">{{ __('country.longitude') }}</label>
                                                    <div class="col-md-9">
                                                        <input class="form-control" name="lng" type="number" />
                                                    </div>
                                                </div>
                                                <div class="form-group">
                                                    <button class="btn btn-info" type="submit">{{ __('country.add') }}</button>
                                                    <button type="button" class="btn btn-default" data-dismiss="modal">{{ __('country.close') }}</button>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
                            ...
                            <table id="datatable-colvid" class="table table-striped table-bordered">
                                ...
                                <tr>
                                    <th>#</th>
                                    <th>{{ __('country.name_arabic') }}</th>
                                    <th>{{ __('country.name_english') }}</th>
                                    <th>{{ __('country.latitude') }}</th>
                                    <th>{{ __('country.longitude') }}</th>
                                    <th>{{ __('country.buttons') }}</th>
                                </tr>
                                </thead>
                                <tbody>
                                @foreach($areas as $i=>$area)
                                    <tr>
                                        ...
                                        <td>{{ $area->lng }}</td>
                                        <td>
                                            <a href="{{route('area.city',$area->id)}}" class="btn btn-info">{{ __('country.area') }}</a>
                                            <a  data-target="#edit_modal{{$i}}" data-toggle="modal" class="btn btn-info"> {{ __('country.edit') }}</a>
                                            <!-- Edit Modal -->
                                            <div class="modal fade" id="edit_modal{{ $i }}" role="dialog">
                                                <div class="modal-dialog ">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <button type="button" class="close" data-dismiss="modal">&times;</button>
                                                            <h4 class="modal-title">{{ __('country.modal_title_edit', ['region_name' => $area->name_ar]) }}</h4>
                                                        </div>
                                                        <div class="modal-body">
                                                            <form method="post" action="{{ route('area.update', $area->id) }}">
                                                                ...
                                                                <div class="form-group">
                                                                    <button class="btn btn-info" type="submit">{{ __('country.edit') }}</button>
                                                                    <button type="button" class="btn btn-default" data-dismiss="modal">{{ __('country.close') }}</button>
                                                                </div>
                                                            </form>
                                                        </div>
                                            ...
                                            <a href="#" data-target="#del_modal{{ $i }}" data-toggle="modal" class="btn btn-danger">{{ __('country.delete') }}</a>
                                            <!-- Delete Modal -->
                                            <div class="modal fade" id="del_modal{{ $i }}" role="dialog">
                                                <div class="modal-dialog ">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <button type="button" class="close" data-dismiss="modal">&times;</button>
                                                            <h4 class="modal-title">{{ __('country.modal_title_delete', ['region_name' => $area->name_ar]) }}</h4>
                                                        </div>
                                                        <div class="modal-body">
                                                            <h2>{{ __('country.confirm_delete') }}<h2>
                                                                    <form method="post" action="{{ route('area.destroy', $area->id) }}">
                                                                        @csrf
                                                                        <input type="hidden" name="_method" value="delete">
                                                                        <div class="form-group">
                                                                            <button class="btn btn-danger" type="submit">{{ __('country.delete') }}</button>
                                                                            <button type="button" class="btn btn-default" data-dismiss="modal">{{ __('country.close') }}</button>
                                                                        </div>
                                                                    </form>
                                                        </div>
